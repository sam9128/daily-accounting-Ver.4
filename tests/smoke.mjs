...
await page.getByRole('button', { name: '上一日' }).waitFor();
await page.getByRole('button', { name: '下一日' }).waitFor();
await page.getByRole('button', { name: '上一日' }).click();
await page.getByText('2026 年 7 月 21 日', { exact: true }).waitFor();
const dailyFoodStats = page.locator('.stats-dialog .stat-row').filter({ hasText: '食' }).first();
await dailyFoodStats.click();
await page.locator('.stats-dialog .category-details').getByText('自動測試食', { exact: true }).waitFor();
await dailyFoodStats.click();
if (process.env.DAY_STATS_SHOT) await page.screenshot({ path: process.env.DAY_STATS_SHOT, fullPage: true });
await page.setViewportSize({ width: 423, height: 822 });
const mobileModeTabs = await page.locator('.mode-tabs').evaluate(element => {
  const tabs = element.getBoundingClientRect();
  const header = element.closest('.stats-header').getBoundingClientRect();
  return { buttons: element.querySelectorAll('button').length, contained: tabs.left >= header.left && tabs.right <= header.right && tabs.top >= header.top && tabs.bottom <= header.bottom };
});
if (mobileModeTabs.buttons !== 4 || !mobileModeTabs.contained) throw new Error(`Mobile day/month/year/flow tabs overflow: ${JSON.stringify(mobileModeTabs)}`);
if (process.env.DAY_STATS_MOBILE_SHOT) await page.screenshot({ path: process.env.DAY_STATS_MOBILE_SHOT, fullPage: true });
await page.setViewportSize({ width: 1440, height: 960 });
