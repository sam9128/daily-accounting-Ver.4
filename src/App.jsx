...
export default function App() {
  ...
  const moveCatalogOrder = async (type, id, direction) => {
    const key = type === 'account' ? 'accounts' : 'categories';
    const reordered = moveCatalogEntry(catalog[key], id, direction);
    if (reordered === catalog[key]) return false;
    await persistCatalog({ ...catalog, updatedAt: new Date().toISOString(), [key]: reordered });
    return true;
  };
